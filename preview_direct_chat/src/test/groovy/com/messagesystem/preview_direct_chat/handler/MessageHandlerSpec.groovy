...
import com.messagesystem.preview_direct_chat.PreviewDirectChatApplication
...
@SpringBootTest(classes = PreviewDirectChatApplication, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
class MessageHandlerSpec extends Specification {
    ...
    def "Direct Chat Basic Test"() {
        ...
        def leftWebSocketSession = leftClient.execute(new TextWebSocketHandler(){
            @Override
            protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Executors{
